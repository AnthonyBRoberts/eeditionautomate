...
class SimpleSubscriberAdmin(admin.ModelAdmin):
    ...
    #fieldsets = [
        ...
        #('system information', {'fields': ['date_created', 'sub_startdate', 'sub_type',]}),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "sub_type":
            ...
            #assert False, SimpleSubscriber.sub_type.filter(request.user
        return super(SimpleSubscriberAdmin, self).formfield_for_foreignkey(db_field, request, **kwargs)

    def get_form(self, request, obj=None, **kwargs):
        if not request.user.is_superuser:
            kwargs['fields'] = ['name', 'address', 'city', 'state', 'zipcode', 'email', 'date_created', 'sub_startdate', 'sub_type',]
        else:
            kwargs['fields'] = ['name', 'address', 'city', 'state', 'zipcode', 'email', 'date_created', 'sub_startdate', 'sub_type', 'publisher',]
